...
public final class CommandRunner {
    ...
    public static ObjectNode getNotifications(final CommandInput commandInput) {
        ...
        objectNode.put("user", commandInput.getUsername());
        objectNode.put("timestamp", commandInput.getTimestamp());
        UserAbstract userAbstract = admin.getAbstractUser(commandInput.getUsername());
        List<Notification> notifications = userAbstract.getNotifications();
    ...
    public static ObjectNode seeMerch(final CommandInput commandInput) {
        ...
        objectNode.put("timestamp", commandInput.getTimestamp());
        User user = admin.getUser(commandInput.getUsername());
        List<String> results = user.seeMerch();
    ...
    public static ObjectNode updateRecommendations(final CommandInput commandInput) {
        ...
        String message;
        if (user != null) {
        message = user.updateRecommendation(commandInput);
        } else {
            message = null;
        }
        objectNode.put("message", message);
    ...
    public static ObjectNode loadRecommendations(final CommandInput commandInput) {
        ...
        String message;
        if (user != null) {
            message = user.loadRecommendation();
        } else {
            message = null;
        }
        objectNode.put("message", message);
...
public static ObjectNode wrapped(final CommandInput commandInput) {
    ...
    for (User user : admin.getUsers()) {
        if (user.getUsername().equals(ourUser)) {
            ...
            for (Artist artist : topArtistsUser)
                topArtistsNode.put(artist.getUsername(), artist.getLoadsNumber());
            ...
            for (Song song : topSongs)
                if (song != null && song.getListens() != 0)
    ...
    for (Artist artist : admin.getArtists()) {
        if (artist.getUsername().equals(ourUser)) {
            WrapperArtist wrapperArtist = new WrapperArtist();
            wrapperArtist.wrapperArtist(artist);
            ...
            for (UserAbstract fan : topFans) {
                topFansArray.add(fan.getUsername());
            }
            resultNode.set("topAlbums", topAlbumsNode);
            resultNode.set("topSongs", topSongsNode);
            resultNode.set("topFans", topFansArray);
            resultNode.put("listeners", wrapperArtist.getTopFans().size());
            break;
        }
    }
    // wrapped host
    ...
    for (endProgramOutput endProgram : endProgramList) {
        ...
        artistNode.put("ranking", endProgram.getRanking());
        artistNode.put("mostProfitableSong", endProgram.getMostProfitableSong());

        resultNode.set(endProgram.getArtistName(), artistNode);
    }
