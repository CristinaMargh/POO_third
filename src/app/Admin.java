...
public final class Admin {
    ...
    public String addAlbum(final CommandInput commandInput) {
        ...
        for (User user : users) {
            ...
            for (ContentCreator contentCreator : user.getCreators()) {
                if (username.equals(contentCreator.getUsername())) {
                    notifications.add(new Notification("New Album", "New Album from "
                            + contentCreator.getUsername() + "."));
                }
            }
    ...
    public String addEvent(final CommandInput commandInput) {
        ...
        for (User user : users) {
            ...
            for (ContentCreator contentCreator : user.getCreators()) {
                if (username.equals(contentCreator.getUsername())) {
                    notifications.add(new Notification("New Event", "New Event from "
                            + contentCreator.getUsername() + "."));
                }
            }
            user.updateNotification(notifications);
    ...
    public String addMerch(final CommandInput commandInput) {
        ...
        for (User user : users) {
            ...
            for (ContentCreator contentCreator : user.getCreators()) {
                if (username.equals(contentCreator.getUsername())) {
                    notifications.add(new Notification("New Merchandise", "New Merchandise from "
                            + contentCreator.getUsername() + "."));
                }
            }
            user.updateNotification(notifications);
...
public ArrayList<endProgramOutput> endProgram() {
    ...
    List<Artist> sortedArtists = artists.stream()
            ...
            .sorted(Comparator.comparingDouble(Artist::getMerchRevenue).reversed()
                    .thenComparing(artist -> artist.getUsername().toLowerCase()))
            .toList();
